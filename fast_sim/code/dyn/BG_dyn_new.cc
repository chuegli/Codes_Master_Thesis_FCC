...
// Command: root -q "./code/dyn/BG_dyn_new.cc+(\"multijets\")"
////////////////////////////////////////////////////////////////////////////////
...
void BG_dyn_new(string BGstring) {
  //////////////////////////////////////////////////////////////////////////////
  ////////// Definitions:
  ...
  // "15000_25000", "25000_35000", "35000_100000"};
  string e[] = {"7200_15000","15000_25000", "25000_35000", "35000_100000"};
  ...
  double de[] = {474.9, 7.349, 0.1759, 0.007654};
  double *myxsec[5] = {da,db,dc,dd,de};

  // int mylength[5] = {7,7,10,1,9};
  int mylength[5] = {7,7,10,1,4};
...
void make_hist(TChain* myChain_t, string BGstring_t, string HTrange_s_t, double ntot_t){
  ...
  while(reader.Next()){
      ...
      double jet_pt[4]    = {0,0,0,0};
      double A=0,B=0,C=0,D=0;
      double jet_phi[4]   = {0,0,0,0};
      double jet_eta[4]   = {0,0,0,0};
      double jet_theta[4] = {0,0,0,0};
      ...
      for(int i=0; i<n_jets; i++){
          ...
          if(abs(p4.Pt())>jet_pt[0]){
              ...
              jet_eta[1]   = jet_eta[0];
              jet_eta[0]   = p4.Eta();
              jet_theta[3] = jet_theta[2];
              jet_theta[2] = jet_theta[1];
              jet_theta[1] = jet_theta[0];
              jet_theta[0] = p4.Theta();
          }else if(abs(p4.Pt())>jet_pt[1] && abs(p4.Pt())<jet_pt[0]){
            jet_pt[3]    = jet_pt[2];
            jet_pt[2]    = jet_pt[1];
            jet_pt[1]    = abs(p4.Pt());
            jet_phi[3]   = jet_phi[2];
            ...
            jet_theta[3] = jet_theta[2];
            jet_theta[2] = jet_theta[1];
            jet_theta[1] = p4.Theta();
          }else if(abs(p4.Pt())>jet_pt[2] && abs(p4.Pt())<jet_pt[0] && abs(p4.Pt())<jet_pt[1]){
            jet_pt[3]    = jet_pt[2];
            jet_pt[2]    = abs(p4.Pt());
            jet_phi[3]   = jet_phi[2];
            jet_phi[2]   = p4.Phi();
            ...
            jet_eta[2]   = p4.Eta();
            jet_theta[3] = jet_theta[2];
            jet_theta[2] = p4.Theta();
          }else if(abs(p4.Pt())>jet_pt[3] && abs(p4.Pt())<jet_pt[0] && abs(p4.Pt())<jet_pt[1] && abs(p4.Pt())<jet_pt[2]){
            jet_pt[3]    = abs(p4.Pt());
            jet_phi[3]   = p4.Phi();
            jet_eta[3]   = p4.Eta();
            jet_theta[3] = p4.Theta();
          }
          delta_phi = min(delta_phi,dphi(p4.Phi(),met.Phi()));
      }
      jet_leading_pt = jet_pt[0];
      jet_leading_eta = jet_eta[0];
      if(c3 && abs(delta_phi) > 0.5){h_LgenJet_pt->Fill(jet_leading_pt);}
      if(c3 && abs(delta_phi) > 0.5){h_LgenJet_eta->Fill(jet_leading_eta);}

      if(c3 && met.Mod()>3500 && abs(delta_phi) > 0.5){
          cout << "Events number: " << counter << endl;
          cout << "1. leading jet: pt, phi, eta, theta: " << jet_pt[0] << ", " << jet_phi[0] << ", " << jet_eta[0] << ", " << jet_theta[0] << endl;
          cout << "2. leading jet: pt, phi, eta, theta: " << jet_pt[1] << ", " << jet_phi[1] << ", " << jet_eta[1] << ", " << jet_theta[1] << endl;
          cout << "3. leading jet: pt, phi, eta, theta: " << jet_pt[2] << ", " << jet_phi[2] << ", " << jet_eta[2] << ", " << jet_theta[2] << endl;
          cout << "4. leading jet: pt, phi, eta, theta: " << jet_pt[3] << ", " << jet_phi[3] << ", " << jet_eta[3] << ", " << jet_theta[3] << endl;
      }
