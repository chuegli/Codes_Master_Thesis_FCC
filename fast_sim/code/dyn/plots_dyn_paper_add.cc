...
for (int i=0; i<2; i++){
      ...
      cout << "i: " << i << endl;
      string filename;
      if(i==0){filename = "/afs/cern.ch/user/c/chugli/private/fast_sim/histos/histo_dyn_c3_higgsino_" +ftype+"_"+f_middle[i]+".root";}
      else{filename = "/afs/cern.ch/user/c/chugli/private/fast_sim/histos/histo_dyn_c3_" +ftype+"_"+f_middle[i]+".root";}
      const char* dirname = filename.c_str();
      TFile *f = new TFile(dirname);

      string chargname[2] = {"Higgsino", "Wino"};
      string chargname2[2] = {"1", "3"};
      string name11 = chargname[i]+" x 1000 "+", m = "+chargname2[i]+" TeV";
      const char* name1 = name11.c_str();
      string name22 = chargname[i]+", m = "+chargname2[i]+" TeV";
      ...
      TH1D* c2_hh_pt      = (TH1D*)f2->Get("hh_pt");
      TH1D* c2_hh_met     = (TH1D*)f2->Get("hh_met");
      TH1D* c2_hh_pt_tot  = (TH1D*)f2->Get("hh_pt_tot");
      TH1D* c2_hh_met_tot = (TH1D*)f2->Get("hh_met_tot");
      TH1D* c2_h_pt_n     = (TH1D*)f2->Get("h_pt_n");
      TH1D* c2_h_met_n    = (TH1D*)f2->Get("h_met_n");

      //////
      c2_h_met_n->Scale(1000);
      c2_h_pt_n ->Scale(1000);
      //////

      HStack_h["h_met"]     ->Add(c2_h_met);
      HStack_h["h_pt"]      ->Add(c2_h_pt);
      HStack_h["hh_pt"]     ->Add(c2_hh_pt);
      HStack_h["hh_met"]    ->Add(c2_hh_met);
      HStack_h["hh_pt_tot"] ->Add(c2_hh_pt_tot);
      HStack_h["hh_met_tot"]->Add(c2_hh_met_tot);
      HStack_h["h_pt_n"]    ->Add(c2_h_pt_n);
      HStack_h["h_met_n"]   ->Add(c2_h_met_n);

      ///// End:
      h_map[index] = HStack_h;
      index++;
} // end of i (loop over {wino,higgsino})
  ...
  for(int i=0; i<4;i++){ // put <5 to include multijets
  ...
  h_map[0]["h_pt_n"]->Draw("same hist h"); // higgsino 1 TeV
  h_map[1]["h_pt_n"]->Draw("same hist h"); // wino 3 TeV
  canvas->BuildLegend(0.4,0.55,0.87,0.87); //x1,y1,x2,y2
  gPad->RedrawAxis();
  gPad->SetTicks(1,1);
  gPad->Modified();
  canvas->SaveAs("/afs/cern.ch/user/c/chugli/private/fast_sim/plots/dyn/plots_dyn_c3_with_higgsino.pdf[");
  canvas->SaveAs("/afs/cern.ch/user/c/chugli/private/fast_sim/plots/dyn/plots_dyn_c3_with_higgsino.pdf");

  s_h_met_n->SetMaximum(1e17);
  ...
  h_map[0]["h_met_n"]->Draw("same hist h"); // higgsino 1 TeV
  h_map[1]["h_met_n"]->Draw("same hist h"); // wino 3 TeV
  ...
  h_map[0]["hh_pt_tot"]->Draw("same h"); // higgsino 1 TeV
  h_map[1]["hh_pt_tot"]->Draw("same h"); // wino 3 TeV
  ...
  h_map[0]["hh_met_tot"]->Draw("same h"); // higgsino 1 TeV
  h_map[1]["hh_met_tot"]->Draw("same h"); // wino 3 TeV
  canvas->BuildLegend(0.45,0.55,0.87,0.87);
  gPad->RedrawAxis();
  gPad->SetTicks(1,1);
  gPad->Modified();
