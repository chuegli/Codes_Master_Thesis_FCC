...
// scp plots_kin.cc chugli@lxplus.cern.ch:/afs/cern.ch/user/c/chugli/private/fast_sim/code/kinematic_selection/
...
void make_arrays_wino(){
  ...
  for(int i=0; i<73;i++){
      for(int j=0; j<73;j++){
          ...
          n_sig[0][counter] = h_map[0]->Integral(a,1e3,b,1e3);
          n_sig[1][counter] = h_map[1]->Integral(a,1e3,b,1e3);
          n_sig[2][counter] = h_map[2]->Integral(a,1e3,b,1e3);
          n_sig[3][counter] = h_map[3]->Integral(a,1e3,b,1e3);
          Double_t error_1 = 0;
          h_map[0]->IntegralAndError(a,1e3,b,1e3,error_1);
          Double_t error_2 = 0;
          h_map[1]->IntegralAndError(a,1e3,b,1e3,error_2);
          Double_t error_3 = 0;
          h_map[2]->IntegralAndError(a,1e3,b,1e3,error_3);
          Double_t error_4 = 0;
          h_map[3]->IntegralAndError(a,1e3,b,1e3,error_4);
          ...
          ostringstream streamObj8;
          streamObj8 << error_0;
          string e0 = streamObj8.str();

          ostringstream streamObj9;
          streamObj9 << error_1;
          string e1 = streamObj9.str();

          ostringstream streamObj10;
          streamObj10 << error_2;
          string e2 = streamObj10.str();

          ostringstream streamObj11;
          streamObj11 << error_3;
          string e3 = streamObj11.str();

          ostringstream streamObj12;
          streamObj12 << error_4;
          string e4 = streamObj12.str();

          mystring = s1+", "+s2+", "+s3+"+/-"+e0+", "+s4+"+/-"+e1+", "+s5+"+/-"+e2+", "+s6+"+/-"+e3+", "+s7+"+/-"+e4+";"+"\n";
          MyFile << mystring;
...
void make_plots_wino(double fake_rate){
  ...
  while (getline (MyReadFile2, s)) {
    if(counter>0 && counter < 5330){
      ...
      n_sig[i] = stod(s5)+stod(s7); /// multipied times the reconstruction efficiency
...
void make_arrays_higgsino(){
  ...
  cout << "TEST: " << h_map[0]->Integral(0,1e3,0,1e3) << ", " << h_map[1]->Integral(0,1e3,0,1e3) << endl;
  ...
  for(int i=0; i<73;i++){
      for(int j=0; j<73;j++){
          ...
          // cout << "selection: " << a << ", " << b << endl;
          cout << "signal: " << n_sig[0][counter] << "+/-"<<error_1 << ", " << n_sig[1][counter] << "+/-"<<error_2 << endl;
          ...
          ostringstream streamObj7;
          streamObj7 << error_1;
          string e5 = streamObj7.str();

          ostringstream streamObj8;
          streamObj8 << error_0;
          string e0 = streamObj8.str();

          mystring = s1+", "+s2+", "+s3+"+/-"+e0+", "+s4+ "+/-"+e4+", "+s5+ "+/-"+e5+";"+"\n";
          MyFile << mystring;
...
void make_plots_higgsino(double fake_rate){
  ...
  while (getline (MyReadFile2, s)) {
    if(counter>0 && counter < 5330){
      ...
      n_sig[i] = stod(s4)+stod(s5); /// multipled times the reconstruciton efficiecy
      z[i] = n_sig[i]/(sqrt(n_BG[i]));
