...
// 2) Write the command: root -q "./code/kinematic_selection/BG_kin.cc+(\"W+jets\")"
////////////////////////////////////////////////////////////////////////////////
...
void make_hist(TChain* myChain_t, string BGstring_t, string HTrange_s_t, double ntot_t){
  ...
  TH2D* h2 = new TH2D("h2", "h2", 1e3,0,1e4,1e3,0,1e4); // histo_kin_
  TH2D* hh2 = new TH2D("hh2", "hh2", 1e3,0,1e4,1e3,0,1e4); // histo_kin_
  ...
  while(reader.Next()){
      ...
      if(c3 && abs(delta_phi)>0.5){h2->Fill(jet_leading_pt,met.Mod());}
  ...
  string rootname = "/afs/cern.ch/user/c/chugli/private/fast_sim/histos/histo_kin_" +BGstring_t+"_"+HTrange_s_t+".root";
