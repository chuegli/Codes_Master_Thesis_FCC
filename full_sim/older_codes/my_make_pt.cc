...
void my_make_histo(string rootname, string outputname, double pt_true, double res_z, bool wino);
...
int main(int argc, char **argv){
    ...
    my_make_histo(filename, outputname, pt_true, res_z, wino);
...
void my_make_histo(string rootname, string outputname, double pt_true, double res_z, bool wino){
  ...
  // TH1D* b_h_pt = new TH1D("b_h_pt", "b_h_pt", 100,0,100);
  ...
  TH1D* h_qOverP = new TH1D("h_qOverP", "h_qOverP", 100,-5,5);
  TH1D* h_Tpt = new TH1D("h_Tpt", "h_Tpt", 100,-500,5000);
  // TH1D* h_Tpt = new TH1D("h_Tpt", "h_Tpt", 100,0,100);

  TH1D* h_Delta_q_pt  = new TH1D("h_Delta_q_pt",  "h_Delta_q_pt",  200,-0.025,0.025);
  ...
  double wino_mass;
  if(wino){
    wino_mass = 3000.000+0.000165; //GeV (better cause beta sim 1 comma ca)
  }else{
    wino_mass = 0.1056583745;
  }
