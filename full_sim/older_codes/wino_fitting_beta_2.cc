...
auto make_fit(TH1D* c_h_beta, bool plot, string nr){
  ...
  if(plot){
    ...
    TF1 *myfit5 = new TF1("myfit5","[0]*exp(-0.5*((x-[1])/[2])^2/(sqrt(2*3.1415926535897932384626)*[2]))",-0.5,0.5);
    myfit5->SetParameters(30,c_h_beta->GetMean(),c_h_beta->GetRMS());
    c_h_beta->Fit("myfit5","b v", "h");
    TF1* myf5 = c_h_beta->GetFunction("myfit5");
    cout << "ndf: "  << myf5->GetNDF() << endl;
...
void make_fit_beta(){
  ...
  string rootname = "/afs/cern.ch/work/c/chugli/private/FCCSW_2/charginos/root_files_size_2/winos_beta_2.root";
  ...
  string ss2 = "plots/wino_fit_size_2_beta_resolutions.pdf";
